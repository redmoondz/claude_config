...
if ! (unalias rg 2>/dev/null; command -v rg) >/dev/null 2>&1; then
  ...
  if [[ -n $ZSH_VERSION ]]; then
    ARGV0=rg /home/rdmoon/.local/share/claude/versions/2.1.81 "$@"
  elif [[ "$OSTYPE" == "msys" ]] || [[ "$OSTYPE" == "cygwin" ]] || [[ "$OSTYPE" == "win32" ]]; then
    ARGV0=rg /home/rdmoon/.local/share/claude/versions/2.1.81 "$@"
  elif [[ $BASHPID != $$ ]]; then
    exec -a rg /home/rdmoon/.local/share/claude/versions/2.1.81 "$@"
  else
    (exec -a rg /home/rdmoon/.local/share/claude/versions/2.1.81 "$@")
  fi
}
fi
export PATH=/home/rdmoon/.local/bin\:/usr/local/sbin\:/usr/local/bin\:/usr/sbin\:/usr/bin\:/sbin\:/bin\:/usr/lib/jvm/default/bin\:/usr/bin/site_perl\:/usr/bin/vendor_perl\:/usr/bin/core_perl\:/home/rdmoon/.fzf/bin\:/home/rdmoon/.lmstudio/bin
